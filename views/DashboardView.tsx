    ...
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      {showDemoModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 no-print">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl text-center border relative overflow-hidden">
             ...
             <div className="w-16 h-16 bg-brand-100 rounded-full flex items-center justify-center mx-auto mb-6"><Lock className="text-brand-600" size={32} /></div>
             <h2 className="text-2xl font-black mb-2">Unlock Full Analysis</h2>
             <p className="text-slate-500 mb-8">Create a <b>free account</b> to run live, deep-dive research on real prospects immediately.</p>
          ...
          {showAdminPanel && (
            <div className="overflow-x-auto bg-slate-800 rounded-xl border border-slate-700">
              <table className="w-full text-left text-sm">
                <thead><tr className="bg-slate-900/50 text-slate-500 uppercase text-xs"><th className="p-3">User</th><th className="p-3">Usage</th><th className="p-3 text-right">Action</th></tr></thead>
                <tbody>{adminUsers.map(u => (<tr key={u.id} className="border-t border-slate-700">
                  <td className="p-3">{u.email}</td><td className="p-3 text-yellow-400">{u.usageCount || 0}</td>
                  <td className="p-3 text-right"><button onClick={() => giftCredits(u.id)} className="text-emerald-400 border border-emerald-500/20 px-2 py-1 rounded text-xs">Gift Pro</button></td>
                </tr>))}</tbody>
              </table>
            </div>
          )}
      ...
      {!isDemoMode && (
          <div className="bg-brand-50 border border-brand-100 p-4 rounded-2xl flex items-center gap-4 no-print">
              ...
              <div className="flex-1">
                  <h3 className="text-xs font-bold text-brand-900 uppercase">My Business Name (For Signature)</h3>
                  <input className="w-full bg-transparent border-b border-brand-200 outline-none text-sm font-medium text-brand-800" value={userStats.businessName} onChange={async (e) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
            ...
            {loading && (<div className="h-full flex flex-col items-center justify-center text-slate-500 bg-white rounded-2xl border min-h-[400px]"><Loader2 size={48} className="animate-spin text-brand-500 mb-4" /><p className="font-bold">Scanning the live web...</p></div>)}
            
            {dossier && (
                <div className="space-y-6 animate-fade-in-up">
                    <div className="flex justify-end gap-3 no-print">
                        <button onClick={() => isPro ? handlePrint() : window.confirm("Exporting to PDF is a Pro Feature.") && (window.location.href = 'https://buy.stripe.com/bJeaEW6Xf2kp5Zc3qAdAk03')} className={`px-4 py-2 rounded-xl font-bold flex items-center gap-2 border transition-all ${isPro ? "bg-slate-100 text-slate-600 border-slate-200" : "bg-slate-50 text-slate-400 border-slate-100"}`}>{isPro ? <Printer size={16} /> : <Lock size={14} />}{isPro ? "Save PDF" : "Unlock PDF"}</button>
                    ...
                    <div ref={componentRef} className="print-container">
                      <div className="hidden print:block mb-8 border-b pb-4"><h1 className="text-3xl font-black">{name}</h1><p className="text-slate-500">{role} at {company}</p></div>
                      
                      <div className="bg-white p-6 rounded-2xl border-l-4 border-brand-500 shadow-sm mb-6 print:border"><div className="flex items-center gap-2 mb-3 text-brand-600"><BrainCircuit size={20} /><h4 className="text-[10px] font-black uppercase tracking-widest">Psychological Profile</h4></div><p className="text-slate-700 leading-relaxed">{renderSafe(dossier.personality)}</p></div>
                      ...
                      <div className="bg-slate-900 p-8 rounded-2xl shadow-xl print:bg-white print:text-black print:border">
                          <div className="flex items-center gap-2 mb-6 text-slate-400 border-b border-slate-800 pb-4 print:border-slate-200 print:text-black"><Mail size={20} /><h4 className="text-[10px] font-black uppercase tracking-widest">Draft Email</h4></div>
                          <div className="text-slate-300 font-mono text-sm whitespace-pre-wrap print:text-black">
                              ...
                              <div className="mt-8 pt-6 border-t border-slate-800 print:border-slate-200">
                                  <p className="text-slate-500 text-xs uppercase tracking-widest mb-1">Sent via Sentient AI</p>
